<?php
    ...
    try {
        if ($isMonthlyQuery) {
            ...
            $sql1 = "SELECT `t_id`,`transactionTypeId`,`transactionDate`, 
                    CASE
                        WHEN `transactionTypeId` = 6 THEN '收入'      -- 綠線
                        WHEN `transactionTypeId` = 5 THEN '支出'      -- 紅線
                        WHEN `transactionTypeId` IN (7, 8, 9) THEN '其他' -- 藍線
                        ELSE '未知'
                    END AS `transactionTypeGroup` FROM `transactions` 
                    WHERE `userId` = :userId AND YEAR(transactionDate) = :year AND MONTH(transactionDate) = :month
                    GROUP BY`transactionTypeGroup`,DATE(transactionDate);";
