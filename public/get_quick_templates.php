...
try {
    ...
    if (!$userInfo || !isset($userInfo['userId']) || (int)$userInfo['userId'] <= 0) {
        add_log($db_log,$userId,-1,"phone-get_quick_templates.php","登入驗證失敗，{$userInfo}");
    ...
    add_log($db_log,$userId,1,"phone-get_quick_templates.php","驗證成功，使用者{$userInfo['username']}");
    ...
    foreach($results as $obj) {
        ...
        $finalOutput[] = [
            'id' => (int)$obj['templateId'],
            'name' => $obj['templateName'],
        ];
    }
    
    // 輸出 JSON
    add_log($db_log,$userId,1,"phone-get_quick_templates.php","獲取快速記帳成功");
    echo json_encode($finalOutput, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);

} catch (PDOException $e) {
    // error_log("Database Error: " . $e->getMessage());
    add_log($db_log,$userId,-1,"phone-get_quick_templates.php","資料庫操作失敗， " . $e->getMessage() . " --- SQLSTATE: " . $e->getCode());
    http_response_code(500);
    // echo json_encode(['status' => 'error', 'message' => '資料庫錯誤。']);
} catch (Exception $e) {
    // error_log("General Error: " . $e->getMessage());
    add_log($db_log,$userId,-1,"phone-get_quick_templates.php","一般伺服器錯誤: ". $e->getMessage());
    http_response_code(500);
